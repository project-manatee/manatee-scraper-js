...
TEAMSParser.NUMERIC_REGEX = /(\d+)/g;
TEAMSParser.EXTRA_CREDIT_REGEX = /^extra credit$|^ec$/ig;
TEAMSParser.EXTRA_CREDIT_NOTE_REGEX = /extra credit/ig;
...
TEAMSParser.prototype.parseClassGrades = function(courseId, semesterIndex, cycleIndex) { 
	...
	var averageMatcher = this.NUMERIC_REGEX.exec($(gradeInfo[3]).text());
	var periodMatcher = this.NUMERIC_REGEX.exec($(gradeInfo[1]).text());
	// parse categories
	var cats = [];
...
TEAMSParser.prototype.parseCategory = function(cat, courseId) {
	...
	var categoryTableId = (catInfo[0].trim().replace(" ", "_") + "BodyTable").trim();
	// Find all of the assignments using category name since assginment table id is CategoryName + "BodyTable"
	var assignments = cat.find('#'+categoryTableId).find("tr");
	
	// parse category average
	var averageMatcher = this.NUMERIC_REGEX.exec(catInfo[1]);

	// parse class weight
	var weightdMatcher = this.NUMERIC_REGEX.exec(catInfo[2]);
	// generate category ID
	var catId = Sha1.hash(courseId + '|' + catInfo[0].trim());
	// parse assignments
...
TEAMSParser.prototype.parseAssignment = function(row, is100Pt, catId) {
	...
	var extraCredit = title.search(this.EXTRA_CREDIT_REGEX) !== -1 || note.search(this.EXTRA_CREDIT_NOTE_REGEX) !== -1;
